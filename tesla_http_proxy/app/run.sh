...
if [ $SERVER_MODE == 1 ]; then
  ...
  tesla-http-proxy -key-file $CONFIG_BASE/tesla/com.tesla.3p.private-key.pem -cert $CONFIG_BASE/tls/tls.crt -tls-key $CONFIG_BASE/tls/tls.key -port 8099 -host 0.0.0.0 -verbose
fi
